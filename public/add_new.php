...
if (isset($_POST["submit"])) {
    ...
    $sqlAmcDetails = "INSERT INTO `amc_details` 
                      (`amcName`, `initiatorName`, `amcCost`, `amcStartDate`, `amcEndDate`, `supplierName`, `amcStatus`) 
                      VALUES (?, ?, ?, ?, ?, ?, ?)";

    $stmtAmcDetails = mysqli_prepare($conn, $sqlAmcDetails);
    mysqli_stmt_bind_param($stmtAmcDetails, "sssssss", $amcName, $initiatorName, $amcCost, $amcStartDate, $amcEndDate, $supplierName, $amcStatus);
    mysqli_stmt_execute($stmtAmcDetails);

    // Get the ID of the inserted row
    $amcId = mysqli_insert_id($conn);

    // Insert phone numbers into phone_numbers table
    $sqlPhone = "INSERT INTO `phone_numbers` (`amc_id`, `phone_number`) VALUES (?, ?)";
    $stmtPhone = mysqli_prepare($conn, $sqlPhone);
    mysqli_stmt_bind_param($stmtPhone, "is", $amcId, $phoneNumber);

    foreach ($phoneNumbersArray as $phoneNumber) {
        mysqli_stmt_execute($stmtPhone);
    }

    // Insert email addresses into emails table
    $sqlEmail = "INSERT INTO `emails` (`amc_id`, `email_address`) VALUES (?, ?)";
    $stmtEmail = mysqli_prepare($conn, $sqlEmail);
    mysqli_stmt_bind_param($stmtEmail, "is", $amcId, $emailAddress);
